...
function answer(i) {
    ...
    resultElement.style.opacity = "0";
    setTimeout(() => {
        resultElement.innerText = answersList[i];
        resultElement.style.opacity = "1";
    }, 150);
}
...
function calculateLove() {
    ...
    const herName = nameInput.value.trim().toLowerCase();
    const resultDiv = document.getElementById('loveResultContainer');
    const percentDisplay = document.getElementById('percent');
    const messageDisplay = document.getElementById('loveMessage');
    ...
    resultDiv.classList.remove('hidden');
    let targetPercent = (herName === "ahnan") ? 100 : Math.floor(Math.random() * 45) + 30;
    let finalMessage = (herName === "ahnan") ? "It's a Perfect Match! ❤️" : "A great match, but something is missing... 🧐";
...
function openLetter() {
    ...
    envelope.style.display = 'none';
    content.classList.remove('hidden');
    for(let i=0; i<15; i++) setTimeout(createFloatingHeart, i * 100);
}
...
function celebrate() {
    ...
    finalPage.style.display = "block";
    audio.src = "./song/Ishqa Ve (1).mp3";
    audio.play().catch(e => console.log("Final song failed"));
    
    // Trigger confetti
    for (let j = 0; j < 50; j++) {
        setTimeout(createConfetti, j * 50);
    }
}
...
function startFireworks() {
    ...
    setTimeout(() => { alert("You made me the happiest person! ❤️"); }, 500);
}